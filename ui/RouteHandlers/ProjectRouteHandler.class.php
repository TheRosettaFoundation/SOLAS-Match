...
error_log("set_memsource_task($task_id... {$part['uid']}...), success: $success");//(**)
            ...
            if ($taskType == Common\Enums\TaskTypeEnum::TRANSLATION && !empty($part['wordsCount'])) $projectDao->increment_used_entitlement($task_id, 0, $part['wordsCount']);
...
//(**)dev server                    $user_id = 3297;
    ...
    public function projectView(Request $request, Response $response, $args)
    {
        ...
        if ($taskDao->isUserRestrictedFromProject($project_id, $user_id) || in_array($project_id, [36065, 36066, 36067, 36068, 36068, 36072, 36070, 36069, 36071, 36073, 36074, 36075, 36076, 36958, 36963])) { // or Moodle
        ...
        if ($roles & (SITE_ADMIN | PROJECT_OFFICER | COMMUNITY_OFFICER | NGO_ADMIN | NGO_PROJECT_OFFICER)) {
            ...
            $extra_scripts .= file_get_contents(__DIR__."/../js/TaskView5.js");
    ...
    public function projectCreate(Request $request, Response $response, $args)
    {
        ...
        $allowed = 1;
        if (!($adminDao->get_roles($user_id, $org_id)&(SITE_ADMIN | PROJECT_OFFICER | COMMUNITY_OFFICER)) && !$projectDao->get_entitlement_remaining($org_id, 0)) {
            $allowed = 0;
            UserRouteHandler::flashNow('error', 'This organization does not have an active package or has used all available quota for new requests. We’d be happy to help you continue! Please get in touch at <a href="mailto:projects@clearglobal.org?subject=Packages" target="_blank">projects@clearglobal.org</a> to discuss your options.');
        }
