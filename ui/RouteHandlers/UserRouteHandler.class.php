...
//(**)incl NGO:            ->add('\SolasMatch\UI\Lib\Middleware:authUserForOrg_incl_community_officer')
    ...
    public function home(Request $request, Response $response, $args = [])
    {
        ...
        $extra_scripts .= "<script type=\"text/javascript\" src=\"{$app->getRouteCollector()->getRouteParser()->urlFor("home")}ui/js/Home5.js\" async></script>";

        $extra_styles = "<link rel=\"stylesheet\" href=\"https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.css\" /><link rel=\"stylesheet\" href=\"{$app->getRouteCollector()->getRouteParser()->urlFor("home")}resources/css/home_styles3.css\" />";
    ...
    public function t_home_ngo(Request $request, Response $response, $args)
    {
        ...
        if ($data = self::t_validate($org_id)) {
            ...
            $data['current_projects'] = $projectDao->get_org_current_projects($org_id);
            $data['completed_files'] = $projectDao->get_org_completed_files($org_id, 6);
            $data['claimed_tasks'] = $claimed_tasks;
            $data['matecat_urls'] = $matecat_urls;
            $data['deadline_timestamps'] = $deadline_timestamps;
            $data['tasks'] = $tasks;
            $data['chunks'] = $chunks;
            $data['org_images'] = $org_images;
            $data['orgs'] = $orgs;
            $data['org_names'] = $org_names;
            $data['news']      = $userDao->get_content_items(null, 1, null, 1, 1, null, null, null, 0, 0);
            $data['resources'] = $userDao->get_content_items(null, 7, null, 1, 1, null, null, null, 0, 0);
        }
    ...
    public function ngo_projects(Request $request, Response $response, $args)
    {
        ...
        $template_data = array_merge($template_data, ['current_page' => 'ngo_projects']);
    ...
    public static function t_validate($org_id)
    {
        ...
        if ($_SERVER['HTTP_TWBKEY'] == Common\Lib\Settings::get('tarjimly.twb_key')) {
            ...
            if ($roles&(SITE_ADMIN | PROJECT_OFFICER | COMMUNITY_OFFICER | NGO_ADMIN | NGO_PROJECT_OFFICER)) {
                ...
                return ['user' => $user, 'user_has_active_tasks' => !empty($all_claimed_tasks), 'ngo_orgs' => $ngo_orgs, 'roles' => $roles, 'org_id' => $org_id];
            }
        }
        return [];
    }
    ...
    public function task_stream(Request $request, Response $response)
    {
        ...
        $extra_scripts  = "<script type=\"text/javascript\" src=\"{$app->getRouteCollector()->getRouteParser()->urlFor("home")}ui/js/Parameters.js\"></script>";
        $extra_scripts .= "<script type=\"text/javascript\" src=\"{$app->getRouteCollector()->getRouteParser()->urlFor("home")}ui/js/Home5.js\" async></script>";
        $extra_scripts .= "<script type=\"text/javascript\"  src=\"{$app->getRouteCollector()->getRouteParser()->urlFor("home")}ui/js/pagination_task_stream.js\" defer ></script>";
        ...
        return UserRouteHandler::render('task_stream.tpl', $response);
    }
...
error_log("un/pw verification errno: $errno, responseCode: $responseCode");//(**)
                ...
                if (!$errno && $responseCode == 200) {
                    ...
                    if ($org_id) {
                        ...
                        if (!empty($result)) {
                          $data[0]['organizationId'] = "{$result[0]['t_org_id']}";
                          if (!empty($o_role)) $data[0]['orgRole'] = $o_role;
                        }
                    }
                    $ch = curl_init(Common\Lib\Settings::get('tarjimly.url') . '/api/v3/admins/users/bulk-create');
error_log('un/pw verification bulk-create POST JSON: ' . json_encode($data));//(**)
...
error_log('Google login JSON:' . print_r($json, 1));//(**)
    ...
    public function create_user_or_login($json, $email, $google_login)
    {
        $userDao = new DAO\UserDao();
        $adminDao = new DAO\AdminDao();
        $orgDao = new DAO\OrganisationDao();
            ...
            if (empty($json['role'])) {
                if ($google_login) {
                    ...
                    if ($org_id) {
                        $result = LibAPI\PDOWrapper::call('get_t_org_id', LibAPI\PDOWrapper::cleanse($org_id));
                        if (!empty($result)) {
                            $data['organizationId'] = "{$result[0]['t_org_id']}";
                            if (!empty($o_role)) $data['orgRole'] = $o_role;
                        }
                    }
                }
            } else {
...
error_log("dummy: $dummy");//(**)
    ...
    public static function userPrivateProfile(Request $request, Response $response, $args)
    {
        ...
        $extra_scripts  = '<link href="' . $app->getRouteCollector()->getRouteParser()->urlFor('home') . 'resources/css/home_styles3.css" rel="stylesheet" />';
        $extra_scripts .= '<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery-validate/1.19.2/jquery.validate.min.js" type="text/javascript"></script> ';
        ...
        $(document).ready(function() {
            ...
            // Update tab counter via BS5 shown.bs.tab events
            ...
            document.getElementById("verifications-tab").addEventListener("shown.bs.tab", function() {
                $(".tabcounter").text("3/3");
            });
            $(".tabcounter").text("1/3");
            ...
            $(".nexttab").click(function() {
                ...
                $("#home input").each(function() {
                    if (!validator.element(this) && valid) valid = false;
                });
            ...
            for (var select_count = 0; select_count < userQualifiedPairsCount; select_count++) {
                Count();
                ...
                if (select_count == 0) {
                    var addButton = $("<div class=\"col-md-1\"><input type=\"button\" class=\"add btn btn-sm btn-outline-primary\" id=\"add\" value=\"+\" title=\"Add another translation pair.\" /></div>");
...
error_log("result: $result");//(**)
            ...
            if ($show_create_memsource_user && !empty($post['mark_create_memsource_user'])) {
                if ($memsource_user_uid = $userDao->create_memsource_user($user_id, $show_create_memsource_user)) {
...
foreach ($rows as $index => $row) {
    ...
    $deal_id = $row['deal_id'];
    if (!empty($row['project_t_code'])) $deal_id .= " ({$row['project_t_code']})";
        ...
        if ($request->getMethod() === 'POST') {
            ...
            for ($i = 0; $i < 20; $i++) {
                if (!empty($_FILES['image']['name'][$i]) && empty($_FILES['image']['error'][$i]) && !empty($_FILES['image']['tmp_name'][$i]) && (($data = file_get_contents($_FILES['image']['tmp_name'][$i])) !== false)) {
                  if (in_array($_FILES['image']['type'][$i], ['image/jpeg', 'image/png', 'image/webp'])) {
    ...
    public function content_list(Request $request, Response $response, $args)
    {
        ...
        $extra_scripts  = "<script type=\"text/javascript\" src=\"{$app->getRouteCollector()->getRouteParser()->urlFor("home")}ui/js/Parameters.js\"></script>";
        $extra_scripts .= "<script type=\"text/javascript\" src=\"{$app->getRouteCollector()->getRouteParser()->urlFor("home")}ui/js/Home5.js\" async></script>";
        $extra_styles = "<link rel=\"stylesheet\" href=\"{$app->getRouteCollector()->getRouteParser()->urlFor("home")}resources/css/home_styles3.css\" />";
