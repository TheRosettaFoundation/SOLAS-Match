...
<section class="bg-light-subtle"> 
    ...
    <table class="table table-borderless" >
        ...
        <tbody class="fs-4">
            ...
            {if $project_complete_date['budget_closed']}
            <tr>
                <td colspan="8" align="center">Budget Closed. Total Savings: ${round($project_complete_date['allocated_budget'] - $total_expected_cost + $total_expected_cost_waived, 2)}</td>
            </tr>
            {/if}
        </tbody>
    </table>
     ...
     <br />
        ...
        {if $roles & ($SITE_ADMIN + $PROJECT_OFFICER)}
            <form id="request_quality_checks" class="bg-light-subtle d-flex  flex-column justify-content-center form_action me-2  mb-4 mb-lg-0" method="post" action="{urlFor name="project-view" options="project_id.$project_id"}" >
                ...
                <input type="hidden" name="request_quality_checks" value="" />
                {if isset($sesskey)}<input type="hidden" name="sesskey" value="{$sesskey}" />{/if}
            </form>
        {/if}
        </div>
