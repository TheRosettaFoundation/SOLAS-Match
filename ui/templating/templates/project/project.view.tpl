...
{if $roles & ($SITE_ADMIN + $PROJECT_OFFICER + $COMMUNITY_OFFICER + $NGO_ADMIN + $NGO_PROJECT_OFFICER)}
                  ...
                  <button class="btn btn-primary text-white mt-2 mt-md-0 restrictions" data-bs-target="#exampleModalToggle" data-bs-toggle="modal" language_pair="{$project->getSourceLocale()->getLanguageCode()}|{$languageCountry|replace:',':'_'}">Restrict Task</button>
                  </span>
{/if}
                      </div>
                    {* endofModal *}
