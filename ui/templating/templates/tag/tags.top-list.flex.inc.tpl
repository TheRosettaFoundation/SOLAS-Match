            ...
            {foreach $top_tags as $tag}
                <div class="tag col-12 p-2 text-wrap">
                    ...
                    {assign var="tagId" value=$tag->getId()}
                     <div  class="border border-dark-subtle p-2 rounded-4 text-center"><a href="{urlFor name="tag-details" options="id.$tagId"}" class="text-center p-1 rounded-5 text-muted text-decoration-none ">{$tag_label}</a></div>
                </div>            
            {/foreach}
