    ...
    <div class="col-sm-12 col-md-8 col-lg-9">
                        ...
                        {/if}
                            <div class="d-flex justify-content-between">
                                <div>
                                        ...
                                        <span class="fs-5 bg-primary border-2 border-primary opacity-75 rounded-circle d-inline-block px-3 text-white align-self-start"> ? </span> 
                                        </div>

                                        <div class="d-flex mt-2 mb-2 ">
                                            <button class="rounded-5 bg-greenish border border-0 align-middle ">  <span class="fs-6 p-1 text-white fw-bold">{TaskTypeEnum::$enum_to_UI[$type_id]['type_text']} </span> </button>
                                                {if $task->getWordCount()}
                                                <button type="button" class=" ms-1 rounded-5  bg-quartenary border border-0 align-middle"><div class="fs-6 p-1 text-white fw-bold"> {Localisation::getTranslation('common_word_count')}: <strong>{$task->getWordCount()}</strong> </div> </button>
                                                {/if}
            ...
            <ul class=" flex-row d-flex justify-content-center list-unstyled">
                    ...
                    <li class="m-3 border border-dark-subtle rounded-3 px-2">
                     ...
                     <li class="m-3 border border-dark-subtle rounded-3 px-2" >
                            <a class="page-link" href="{urlFor name="$url_nam" options="page_no.$paged|tt.$selectedTaskType|sl.$selectedSourceLanguageCode|tl.$selectedTargetLanguageCode "}">last</a></li>
            </ul>
