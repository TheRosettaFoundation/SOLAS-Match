{assign var=task_id value=$task->getId()}
    ...
    <section>
            <h2 class="fw-bold">{Localisation::getTranslation('common_what_happens_now')} <small class="text-gray">{Localisation::getTranslation('task_claimed_translation_we_need_your_translation')}</small></h2>
            <p>{Localisation::getTranslation('common_this_is_what_you_need_to_do_as_soon_as_possible')}:</p>
            <ol>
                {if $matecat_url != ''}
                <li>{if !empty($memsource_task)}Please take a look at our <a href="https://community.translatorswb.org/t/the-kato-translators-toolkit/3138" class="fw-bold text-primary" target="_blank">Translator’s Toolkit</a> before working on this task.{else}{Localisation::getTranslation('task_claimed_please_read_kato')}{/if}</li>
