...
<span class="hidden">
  ...
  {assign var="body" value="Dear TWB Translator,\r\n\r\nA new task is available for you on TWB Platform.\r\n\r\nTask name: {$task->getTitle()}\r\nLanguage pair: {TemplateHelper::getLanguageAndCountry($task->getSourceLocale())} to {TemplateHelper::getLanguageAndCountry($task->getTargetLocale())}\r\nType: {foreach from=TaskTypeEnum::$enum_to_UI key=task_type item=ui}{if $type_id == $task_type}{$ui['type_text']} Task{/if}{/foreach}\r\nTask delivery date: {$task->getDeadline()}\r\nWord count: {$task->getWordCount()}\r\n\r\nTo learn more about the task and/or claim this task,\r\nplease visit https://kato.translatorswb.org/task/{$task_id}/view\r\n\r\nDo not start translating until you have claimed the translation\r\non the page above.\r\n\r\nIf you have any questions or comments about this task,\r\nyou can leave a message in the TWB Community forum:\r\nhttps://community.translatorswb.org/t/{$discourse_slug}\r\n\r\n{if !empty($other_task_ids)}Additionally, the above task is part of a project that was split into chunks because of its word count.\r\nOther task chunks you could claim are:\r\n{foreach $other_task_ids as $other_task_id}https://kato.translatorswb.org/task/{$other_task_id}/view\r\n{/foreach}\r\n{/if}Thank you for your contribution and for your continued support!\r\n\r\n"}
