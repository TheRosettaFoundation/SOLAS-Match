...
{if !empty($paid_status)}
            ...
            {if isset($sesskey)}<input type="hidden" name="sesskey" value="{$sesskey}" />{/if}
                ...
                <hr />
                <div>
                   ${round($total_expected_price, 2)} for {if $task->get_word_count_partner_weighted() != '' && $task-> get_word_count_partner_weighted() > 1}{$task-> get_word_count_partner_weighted()}{else}-{/if} {TaskTypeEnum::$enum_to_UI[$type_id]['unit_count_text_short']}
                </div>
