...
{if !empty($paid_status)}
          ...
          {else}
            ...
            <hr />
            <div> 
        ${round({math equation="x*y" x=${TaskTypeEnum::$enum_to_UI[$type_id]['unit_rate']} y={TaskTypeEnum::$enum_to_UI[$type_id]['unit_count_text_short']}}, 2)} for {if $task->getWordCount() != '' && $task->getWordCount() > 1}{$task->getWordCount()}{else}-{/if} {TaskTypeEnum::$enum_to_UI[$type_id]['unit_count_text_short']}
