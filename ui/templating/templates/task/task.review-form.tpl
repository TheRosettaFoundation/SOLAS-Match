<form class="well" method="post" action="{$formAction}"  onsubmit="createHiddenFields()" id="TaskReviewForm" accept-charset="utf-8">
    {foreach $tasks as $task}
        ...
        {if $task->getId() != null}
            ...
            <p>{sprintf(Localisation::getTranslation(Strings::TASK_REVIEW_FORM_0), {urlFor name="download-task-latest-version" options="task_id.$id"})}
            </p>
        {else}
            {assign var="id" value=$task->getProjectId()}
            <p>{sprintf(Localisation::getTranslation(Strings::TASK_REVIEW_FORM_1), {urlFor name="download-project-file" options="project_id.$id"})}
            </p>
        {/if}
