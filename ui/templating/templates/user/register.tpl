...
<body {if isset($body_class)}class="{$body_class}"{/if} {if isset($body_id)}id="{$body_id}"{/if} class="d-flex align-items-center min-vh-100 " >
  ...
  <div class=" row py-2 d-flex justify-content-between  ">
      ...
      <div class="mb-4 mt-4">
              <h2 class="fw-bold">{Localisation::getTranslation('ff_creat_ac')}</h2>
      </div>
      ...
      {if (empty($disabled))}
        <form method="post" id="registerform" action="{urlFor name="register"}" class="wel mt-3" accept-charset="utf-8">
             <div class="d-flex mb-2">
                ...
                <input type="text" name="first_name" id="first_name" placeholder="{Localisation::getTranslation('ff_first')}" class="form-control mb-2" {if isset($first_name)}value="{$first_name|escape:'html':'UTF-8'}"{/if} required/>
                </div>
                
                <div>

                <label for="last_name" class="required  mb-1"><strong>{Localisation::getTranslation('ff_last')}</strong></label>
              <input type="text" name="last_name" id="last_name" placeholder="{Localisation::getTranslation('ff_last')}" class="form-control mb-2" {if isset($last_name)}value="{$last_name|escape:'html':'UTF-8'}"{/if} required/>
            ...
            <input type="text" name="email" id="email" class="form-control mb-2" placeholder="{Localisation::getTranslation('ff_your_email')}" {if isset($email)}value="{$email|escape:'html':'UTF-8'}"{/if} />
              </div>
             <div>
             <label for="password" class="required form-label mb-1"><strong>{Localisation::getTranslation('ff_password')}</strong></label>
             <input type="password" name="password"  class="form-control mb-1" id="password" placeholder="{Localisation::getTranslation('ff_your_pw')}"/>
             </div>
             
             <div>
             <label for="password" class="required form-label  mb-1"><strong>{Localisation::getTranslation('ff_confirm_pw')}</strong></label>
             <input type="password" name="confirm_password" id="confirm_password" class="form-control mb-1" placeholder="{Localisation::getTranslation('ff_please_confirm')}"/>
              ...
              <input name="conduct_consent" id="conduct_consent" class="form-check-input " type="checkbox"> <span class="ms-1 required">{Localisation::getTranslation('ff_i_agree')} </span>
              ...
              <div class="fs-5 text-muted text-center mt-2 "> <a href='{urlFor name="login"}' class="link-grayish link-offset-2 link-offset-3-hover link-underline-grayish link-underline-opacity-0 link-underline-opacity-75-hover "> {Localisation::getTranslation('ff_i_already')}</a> </div>
