   ...
   <div class="container px-4  py-4 pt-5">
     ...
     <div class="d-flex align-items-center ">
                   ...
                   {if $roles & ($SITE_ADMIN + $PROJECT_OFFICER + $COMMUNITY_OFFICER)}
                            <a href="{urlFor name="claimed-tasks" options="user_id.{$this_user->getId()}"}" class="btn-sm btn-primary text-white">
                                <i class="icon-list icon-white"></i> {Localisation::getTranslation('claimed_tasks_claimed_tasks')}
                            </a>
                        {/if}
                        {if $roles & ($SITE_ADMIN + $PROJECT_OFFICER)}
                                <a href="{urlFor name="create-org"}" class="btn-sm btn-success"
                        ...
                        {if $private_access || ($roles & ($SITE_ADMIN + $PROJECT_OFFICER + $COMMUNITY_OFFICER))}
                          {if $admin_role}
                            <a href='{urlFor name="user-code-of-conduct" options="user_id.$user_id"}' class='btn-sm btn-primary text-white'>
                                <i class="icon-wrench icon-white"></i> {Localisation::getTranslation('user_public_profile_edit_profile_details')}
                            </a>
                          {else}
                            <a href='{urlFor name="user-private-profile" options="user_id.$user_id"}' class='btn-sm btn-primary text-white border-bottom-4 border-primaryDark '>
                                <i class="icon-wrench icon-white"></i> {Localisation::getTranslation('user_public_profile_edit_profile_details')}
                            </a>
                          {/if}
                        {/if}
                        {if ($roles & ($SITE_ADMIN + $PROJECT_OFFICER + $COMMUNITY_OFFICER)) && $howheard['reviewed'] == 0}
                            <form method="post" action="{urlFor name="user-public-profile" options="user_id.$user_id"}">
                                <input type="submit" class="btn-sm btn-primary text-white" name="mark_reviewed" value="Mark New User as Reviewed" />
                                {if isset($sesskey)}<input type="hidden" name="sesskey" value="{$sesskey}" />{/if}
                            </form>
                        {/if}
                        {if $show_create_memsource_user}
                            <form method="post" action="{urlFor name="user-public-profile" options="user_id.$user_id"}">
                                <input type="submit" class="btn-sm btn-primary text-white" name="mark_create_memsource_user" value="Create Matching Phrase TMS User" />
                                {if isset($sesskey)}<input type="hidden" name="sesskey" value="{$sesskey}" />{/if}
                            </form>
                        {/if}
    </div> 
...
<table border="0">
    <tr valign="top">
        <td style="width: 30%"><input type='text' value="" name="comment" id="comment" class="form-control-sm" /></td>
        <td style="width: 22%"><input type='text' value="" name="points" id="points" class="form-control-sm" /></td>
        <td style="width: 18%"></td>
        <td style="width: 18%"></td>
        <td style="width: 12%"></td>
    </tr>
