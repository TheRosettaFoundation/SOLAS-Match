                ...
                <td colspan="2">
                    <table>
                        ...
                        {foreach from=$certification_list key=name item=certification}
                            <tr align="center"><td colspan="1" align="center">{if $certification['state']}Already submitted: {/if}<a href="{urlFor name="user-public-profile" options="user_id.$user_id|cert_id.$name"}" target="_blank">{$certification['desc']}</a></td></tr>
                        {/foreach}
                    </table>
                </td>
            </tr>
