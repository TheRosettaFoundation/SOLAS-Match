        ...
        <tr>
            ...
            <td>{if !empty($task['deal_id'])}<a href="{urlFor name="deal_id_report" options="deal_id.{$task['deal_id']}"}" target="_blank">{$task['deal_id']}</a>{/if}
                ...
                {if $task['total_paid_words'] != 0 && !empty($task['deal_id']) && !empty($task['project_t_code']) && !empty($task['purchase_requisition']) && !empty($task['approvalStatus']) && !empty($task['linguist_t_code']) && !empty($task['google_drive_link'])}
                    ...
                    {elseif empty($task['po_created']) && $task['outside_cut_off']=0 && $task['po_create_failed']>0}<span style="color: red;">Purchase Order Creation Failure <a href="{urlFor name="sun_po_errors"}" target="_blank">(log)</a>{if $task['purchase_order'] != '0'}; Old format PO Previously Specified: {$task['purchase_order']}{/if}</span>
            ...
            <td>{substr($task['created-time'], 0, 10)}</td>
