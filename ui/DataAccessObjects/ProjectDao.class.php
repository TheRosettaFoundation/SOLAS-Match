...
GROUP BY c.id, u.id';
            if ($result = $conn->query($sql)) {
                ...
                $result = null;
                $projects = [2 => 36065, 4 => 36066, 5 => 36067, 7 => 36068, 9 => 36068, 11 => 36072, 12 => 36070, 15 => 36069, 16 => 36071, 17 => 36073, 18 => 36074, 20 => 36075, 25 => 36958, 19 => 36963];
                $user_enrolment_ids = [];
                foreach ($data as $row) {
                    if (!empty($row['email'])) {
                        $hash = '';
...
$xml .=
"<PurchaseOrderLine><LineNumber>$line_number</LineNumber><ItemCode>$budget_line</ItemCode>" .
...
error_log("Create PO ref: $result");
    ...
    public function set_entitlement($json)
    {
        ...
        if (empty($json['idempotency_key'])) $json['idempotency_key'] = null;
        $args =
