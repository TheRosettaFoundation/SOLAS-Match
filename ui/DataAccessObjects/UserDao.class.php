        ...
        if ($memsource_project && $memsource_task) {
           ...
           if ($whole_workflow && !$shell_task && !$hook_from_phrase) {
            ...
            if ($cancelled) {
                ...
                if ($status_id < Common\Enums\TaskStatusEnum::IN_PROGRESS && $task->getTaskType() == Common\Enums\TaskTypeEnum::TRANSLATION) LibAPI\PDOWrapper::call('increment_used_entitlement', LibAPI\PDOWrapper::cleanse($task_id) . ',0,' . LibAPI\PDOWrapper::cleanse(-$task->getWordCount()));
            }
        }
...
error_log("TM added: $result");//(**)
        ...
        $sources = ['en_gb', 'en_us'];
        $targets = ['ar_sa', 'es_419', 'es_co', 'es_es', 'fr_ca', 'fr_cd', 'fr_fr', 'pt_br', 'pt_mz', 'pt_pt', 'zh_cn', 'zh_tw', 'ru_ru', 'it', 'de', 'uk_ua', 'ro', 'tr', 'pl', 'hr', 'cs', 'da', 'et', 'fi', 'el', 'hu', 'ga', 'lv', 'lt', 'mt_mt', 'nb', 'sl', 'sk', 'nl', 'sv', 'sq', 'sr_cyrl_rs', 'ko_kr', 'vi_vn', 'bg', 'he', 'id_id', 'th_th', 'hi_in', 'bn_bd', 'bn_in', 'sw', 'ha', 'my_mm'];
        foreach ($langs as $language) {
...
error_log(print_r($result, true));//(**)
    ...
    {
        $this->client->call(null, "{$this->siteApi}v0/users/$user_id/key/" . Common\Lib\Settings::get('tarjimly.api_key')  . '/', Common\Enums\HttpMethodEnum::DELETE);
    }
}
