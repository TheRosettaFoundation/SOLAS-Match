...
function set_errors_for_submission(id) {
	...
	if (alertError != null) {
		...
		html +=
			"<strong>" + parameters.getTranslation("common_error") + ": </strong>";
		html += alertError;
		html += "</p>";
	}
...
function loadingComplete() {
	var userQualifiedPairsCountDatabase = parseInt(
		getSetting("userQualifiedPairsCount")
	);
...
function addSecondaryLanguage(
	...
	if (userQualifiedPairsCount < userQualifiedPairsLimit) {
		...
		text1.innerHTML =
			"<strong>" +
			parameters.getTranslation("i_can_translate_from") +
			":</strong>";
		text1.style.width = "82%";
		locale.appendChild(text1);

		var languageBox = document.createElement("select");
		languageBox.innerHTML = document.getElementById(
		...
		if (userQualifiedPairLanguageCodeSource === undefined)
			userQualifiedPairLanguageCodeSource = "";
		...
		text2.innerHTML =
			"<strong>" + parameters.getTranslation("common_to") + ":</strong>";
		text2.style.width = "82%";
		locale.appendChild(text2);

		var languageBoxTarget = document.createElement("select");
		languageBoxTarget.innerHTML = document.getElementById(
			"template_language_options"
		).innerHTML;
		...
		if (userQualifiedPairLanguageCodeTarget === undefined)
			userQualifiedPairLanguageCodeTarget = "";
		...
		text3.innerHTML =
			"<strong>" +
			parameters.getTranslation("qualification_level_for_above") +
			":</strong>";
		text3.style.width = "82%";
		locale.appendChild(text3);

		var qualificationLevel = document.createElement("select");
		qualificationLevel.innerHTML = document.getElementById(
		...
		if (userQualifiedPairQualificationLevel === undefined)
			userQualifiedPairQualificationLevel = 1;
		qualificationLevel.value = userQualifiedPairQualificationLevel;
		if (!parseInt(getSetting("isSiteAdmin")))
			qualificationLevel.disabled = true;
...
function removeSecondaryLanguage() {
	if (userQualifiedPairsCount > 0) {
		...
		var element = document.getElementById(
			"secondary_locale_" + userQualifiedPairsCount
		);
...
function validateForm() {
	...
	if (!document.getElementById("over18").checked) {
		alertError =
			"You must confirm you are over the age of 18 years to proceed.";
		set_all_errors_for_submission();
		return false;
	}
	...
	if (
		userQualifiedPairsCount == 0 ||
		document.getElementById("language_code_source_0").value == "" ||
		document.getElementById("language_code_target_0").value == ""
	) {
		alertError =
			"You must fill out the languages you can translate from and to.";
		set_all_errors_for_submission();
		return false;
	}
	...
	if (
		document.getElementById("langPrefSelect").value !=
		getSetting("langPrefSelectCodeSaved")
	) {
		window.alert(
			parameters.getTranslation(
				"user_private_profile_language_preference_updated"
			)
		);
	}

	for (var i = 0; i < userQualifiedPairsCount; i++) {
		var languageCodeSource = document.getElementById(
			"language_code_source_" + i
		);
		var languageCodeTarget = document.getElementById(
			"language_code_target_" + i
		);

		if (
			(languageCodeSource.value == "" && languageCodeTarget.value != "") ||
			(languageCodeTarget.value == "" && languageCodeSource.value != "") ||
			languageCodeSource.value == languageCodeTarget.value
		) {
			alertError = parameters.getTranslation(
				"user_private_profile_secondary_languages_failed"
			);
			set_all_errors_for_submission();
			return false;
		}
	...
	if (!checkedCount) {
		alertError =
			"You must indicate that you can provide at least one service such as translation.";
		set_all_errors_for_submission();
		return false;
	}
	...
	$.ajax({
		...
		.done(function (data, textStatus, jqXHR) {
			if (jqXHR.status < 400) {
				...
				functionOnFail();
				console.log(
					"Error: deleteUser returned " + jqXHR.status + " " + jqXHR.statusText
				);
			}
		})
		.fail(functionOnFail);
...
function DAOdestroyUserSession() {
	document.cookie =
		"slim_session=; expires=Thu, 01 Jan 1970 00:00:00 GMT; path=/";
}
