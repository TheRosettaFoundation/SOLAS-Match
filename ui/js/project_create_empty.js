...
function set_errors_for_submission(id, id_for_div) {
    ...
    if (
        ...
        html +=
            '<div id="' + id_for_div + '" class="alert alert-error pull-left">';
        html +=
            "<h3>" +
            parameters.getTranslation("common_please_correct_errors") +
            ":</h3>";
        html += "<ol>";
        if (titleError != null) {
...
function documentReady() {
    ...
    supportedImageFormats = getSetting("supportedImageFormats")
        .toString()
        .split(","); // Image format string is comma separated, split it into a list

    deadline_timestamp = new Date(
        document.getElementById("deadline_timestamp").innerHTML * 1000
    );
    document.getElementById("selectedYear").value =
        deadline_timestamp.getFullYear();
    document.getElementById("selectedMonth").value =
        deadline_timestamp.getMonth() + 1;
    document.getElementById("selectedDay").value = deadline_timestamp.getDate();
    document.getElementById("selectedHour").value =
        deadline_timestamp.getHours();
    document.getElementById("selectedMinute").value =
        deadline_timestamp.getMinutes();
...
function loadingComplete() {
    document.getElementById("image_file_desc").innerHTML =
        parameters.getTranslation("project_create_upload_project_image") +
        " " +
        parameters
            .getTranslation("common_maximum_file_size_is")
            .replace("%s", imageMaxFileSize / 1024 / 1024);
...
function validateForm() {
    ...
    sourceLocale.languageName = $(
        "#sourceLanguageSelect option:selected"
    ).text();
    sourceLocale.languageCode = document.getElementById(
...
function validateLocalValues() {
    ...
    if (title == "") {
        titleError = parameters.getTranslation(
            "project_create_error_title_not_set"
        );
        success = false;
    } else if (title.length > 110) {
        titleError = parameters.getTranslation(
            "project_create_error_title_too_long"
        );
        success = false;
        ...
        titleError = parameters.getTranslation(
            "project_create_title_cannot_be_number"
        );
        success = false;
    ...
    if (description == "") {
        ...
        descriptionError = parameters.getTranslation(
            "project_create_error_description_too_long"
        );
        success = false;
    }
    if (impact == "") {
        ...
        impactError = parameters.getTranslation(
            "project_create_error_impact_too_long"
        );
        success = false;
    }
    if (reference != null && reference != "") {
        if (reference.length > 128) {
            referenceError = parameters.getTranslation(
                "project_create_error_reference_too_long"
            );
            success = false;
        } else if (!validateReferenceURL(reference)) {
            // String did not match pattern, it is not a URL
            referenceError = parameters.getTranslation(
                "project_create_error_reference_invalid"
            );
            success = false;
        }
    }
    ...
    if (!validateTagList(tagList)) {
        ...
        for (var i = 0; i < list.length; i++) {
            if (list[i].length > 50) {
                ...
                tagsError = parameters.getTranslation(
                    "project_create_error_tags_too_long"
                );
                success = false;
                break;
            }
    ...
    if (projectDeadline != null) {
        if (
            ...
            (userIsAdmin ||
                projectDeadline >= deadline_timestamp ||
                document.getElementById("earthquake").checked)
        ) {
            ...
            project.deadline =
                projectDeadline.getUTCFullYear() +
                "-" +
                m +
                "-" +
                d +
            ...
            if (!userIsAdmin)
                deadlineError = "One week is the minimum deadline.";
            success = false;
        }
    } else {
    ...
    if (project.sourceLocale.languageCode == 0) {
        project_create_set_source_language = parameters.getTranslation(
            "project_create_set_source_language"
        );
        success = false;
    }
    if (project.sourceLocale.countryCode == 0) {
        project_create_set_source_country = parameters.getTranslation(
            "project_create_set_source_country"
        );
        success = false;
    }
    return success;
    ...
    if (projectImageFileField != null && files.length > 0) {
        ...
        if (projectImageFile.size > 0) {
            ...
            if (projectImageFile.size <= imageMaxFileSize) {
                ...
                if (extensionStartIndex > 0) {
                    ...
                    if ($.inArray(extension, supportedImageFormats) == -1) {
                        imageError = parameters
                            .getTranslation(
                                "project_create_please_upload_valid_image_file"
                            )
                            .replace("%s", extension);
                        return false;
                    }
                    ...
                    imageError = parameters.getTranslation(
                        "project_create_image_has_no_extension"
                    );
                    return false;
                }
            } else {
                // File is too big
                imageError = parameters.getTranslation(
                    "project_create_image_is_too_big"
                );
                return false;
            }
        } else {
            // File is empty
            imageError = parameters.getTranslation(
                "project_create_image_file_empty"
            );
            return false;
        }
    } else {
...
function set_options_for_days_in_month(month, year) {
    ...
    if (current_selected_day > monthLengths[month])
        current_selected_day = monthLengths[month];

    var options_list = "";
    for (var i = 1; i <= monthLengths[month]; i++) {
        selectedString = "";
        if (initial_selected_day != null && i == initial_selected_day)
            selectedString = ' selected="selected"';
        if (initial_selected_day == null && i == current_selected_day)
            selectedString = ' selected="selected"';
        options_list +=
            '<option value="' +
            i.toString() +
...
function checkTitleNotUsed() {
    if (
        ...
        // Make sure API call will succeed and the field is not empty
        DAOcheckProjectByNameAndOrganisationNotExist(
            document.getElementById("project_title").value,
            org_id,
            noTitleConflict,
            titleConflict,
 ...
 */
function DAOcheckProjectByNameAndOrganisationNotExist(
    title,
    orgId,
    functionNotExist,
    functionExist,
    ...
    $.ajax({
        url:
            siteAPI +
            "v0/projects/getProjectByNameAndOrganisation/" +
            title +
            "/organisation/" +
            orgId +
            "/",
        ...
        .done(function (data, textStatus, jqXHR) {
            if (jqXHR.status < 400) {
                ...
                functionOnFail();
                console.log(
                    "Error: getProjectByName returned " +
                        jqXHR.status +
                        " " +
                        jqXHR.statusText
                );
