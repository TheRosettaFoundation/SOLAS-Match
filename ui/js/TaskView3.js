...
async function configure_buttons() {
  ...
  if (highlight_index >= 4) {
    if (document.getElementById("status_id").innerHTML == 10) {
      document.getElementById("head_show-revision-btn").innerHTML = '<div class="btn-disabled-grey task_tooltip" aria-disabled="true">Work on the task<span class="task_tooltiptext">You must wait for the completion of the previous step before starting on this task.</span></div>';
    } else {
