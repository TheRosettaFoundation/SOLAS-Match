...
class Badges {
    public static function init(){
        ...
        Dispatcher::registerNamed(HttpMethodEnum::POST, '/v0/badges(:format)/', function ($format=".json"){
        ...
        Dispatcher::registerNamed(HttpMethodEnum::PUT, '/v0/badges/:id/', function ($id,$format=".json"){
         ...
         Dispatcher::registerNamed(HttpMethodEnum::DELETE, '/v0/badges/:id/', function ($id,$format=".json"){
            if(!is_numeric($id)&& strstr($id, '.')){
               $id= explode('.', $id);
               $format='.'.$id[1];
               $id=$id[0];
