...
//(**) ALAN Work In Progress
    ...
    @param string $filename is the name of the file being uploaded
    @param int $userId is the id of the user uploading the file
    @return int size of file written, false on failure.
    */
    public static function saveProjectFileToFileSystem($projectId, $file, $filename, $userId)
    {
        $destination = Common\Lib\Settings::get("files.upload_path")."proj-$projectId/";
        if (!file_exists($destination)) {
            mkdir($destination, 0755);
        ...
        $token = DAO\ProjectDao::recordProjectFileInfo($projectId, $filename, $userId, $mime);
        return file_put_contents($destination.$token, $file);
    }
    ...
    @return int size of file written, false on failure.
    */
    public static function saveProjectImageFileToFileSystem($projectId, $file, $filename, $userId)
    {
        ...
        try {
            ...
            $size = file_put_contents($destination."/image.$ext", $file);
